...
// and cmd/vendor.)
func lookupGorootExport(ctx *build.Context, pkgDir string) (string, bool) {
...
func findPkg(ctx *build.Context, path, srcDir string) (filename, id string) {
	...
	default:
		...
		if bp.PkgObj == "" {
			var ok bool
			if bp.Goroot && bp.Dir != "" {
				filename, ok = lookupGorootExport(ctx, bp.Dir)
			}
			if !ok {
...
// importContext imports a gc-generated package given its import path and srcDir, adds
